...
svg('../Data/Figures/Total_nematodes.svg')
...
svg('../Data/Figures/Plant_parasitic_nematodes.svg')
bargraph.CI(data.all$Rate, nem.troph$Para)
dev.off()

svg('../Data/Figures/Fungivorous_nematodes.svg')
bargraph.CI(data.all$Rate, nem.troph$Fung)
dev.off()

svg('../Data/Figures/Bacterivorous_nematodes.svg')
bargraph.CI(data.all$Rate, nem.troph$Bact)
dev.off()

svg('../Data/Figure/Predatory_nematodes.svg')
bargraph.CI(data.all$Rate, nem.troph$Pred)
dev.off()

svg('../Data/Figures/Omnivorous_nematodes.svg')
bargraph.CI(data.all$Rate, nem.troph$Omn)
dev.off()

cld(lsmeans(aov(nem.troph$Pred ~ factor(Rate), data= data.all), ~ factor(Rate), Letters = letters, adjust = "Tukey"))
...
custom.plot.pca(ord = nem.troph.pca, group = factor(data.all$Rate), title = "", plottype = "t")
dev.off()
